...
public class IntervalFloat implements BaseNonRelationalValueDomain<IntervalFloatLattice>{
	...
	public IntervalFloatLattice evalUnaryExpression(UnaryExpression expression, IntervalFloatLattice arg, ProgramPoint pp,
		...
		if(expression.getOperator() == NumericNegation.INSTANCE) {
			...
			return new IntervalFloatLattice(u.multiply(MathNumber.MINUS_ONE),l.multiply(MathNumber.MINUS_ONE));
		}
	...
	public IntervalFloatLattice evalBinaryExpression(BinaryExpression expression, IntervalFloatLattice left,
		...
		if(expression.getOperator() instanceof AdditionOperator) {
			...
			return new IntervalFloatLattice(ladd, uadd);
			...
			IntervalFloatLattice res = new IntervalFloatLattice(p1.min(p2).min(p3).min(p4), p1.max(p2).max(p3).max(p4));
			...
			IntervalFloatLattice res = new IntervalFloatLattice(lsub, usub);
			// System.out.println(res.representation());
			return res;
			// TODO: homework
			...
			if (l2.isZero()) { // [l1,u1] / [0,?]
				if (u2.isInfinite())
					res = IntervalFloatLattice.TOP;
				else if (l1.compareTo(MathNumber.ZERO) >= 0)
					res = new IntervalFloatLattice(u1.divide(u2), MathNumber.PLUS_INFINITY); // [+,+] / [0,+] = [u1/u2, +inf]
				else if (u1.isNegative())
					res = new IntervalFloatLattice(MathNumber.MINUS_INFINITY, l1.divide(u2)); // [-,-] / [0,+] = [-inf, l1/u2]
				else
					res = IntervalFloatLattice.TOP; // [-,+] / [0,+] = [-inf, +inf]
				// System.out.println(res.representation());
				return res;
				...
				if (l2.isInfinite())
					...
					res = new IntervalFloatLattice(MathNumber.MINUS_INFINITY, u1.divide(l2)); // [+,+] / [-,0] = [-inf, u1/l2]
				else if (u1.isNegative())
					res = new IntervalFloatLattice(l1.divide(l2), MathNumber.PLUS_INFINITY); // [-,-] / [-,0] = [l1/l2, +inf]
				else
					res = IntervalFloatLattice.TOP; // [-,+] / [-,0]
				// System.out.println(res.representation());
				return res;
			}
			...
			res = new IntervalFloatLattice(minV, maxV);
			// System.out.println(res.representation());
			return res;
