...
public class Sign implements BaseNonRelationalValueDomain<SignLattice> {
    ...
    /**
     * A constant is also C="a" but we are traching number so we will return SignLattice.TOP.
     * For other values we need to evaluate.
     */
    @Override
    public SignLattice evalConstant(Constant constant, ProgramPoint pp, SemanticOracle oracle) throws SemanticException {

        if(constant.getValue() instanceof Integer){
            // I need to check the integer value to assign the right approximation value.
    ...
    /**
     * SignLattice arg: is the value of the unary operation we received.
     * If the expression is not NegatableOperator (-x) we return TOP.
     */
    @Override
    ...
    public SignLattice evalBinaryExpression(BinaryExpression expression, SignLattice left, SignLattice right, ProgramPoint pp, SemanticOracle oracle) throws SemanticException {
        if(expression.getOperator() instanceof AdditionOperator)
            return evalAdditionSign(left, right);
        else if (expression.getOperator() instanceof  MultiplicationOperator)
            return evalMultiplicationSign(left,right);
        else if (expression.getOperator() instanceof SubtractionOperator)
            return evalSubtractionSign(left,right);
        else if (expression.getOperator() instanceof DivisionOperator)
            return evalDivisionSign(left,right);
        else if (expression.getOperator() instanceof ModuloOperator)
            return evalModuloSign(left,right);
        else if (expression.getOperator() instanceof RemainderOperator)
            return evalRemainderSign(left,right);

        return SignLattice.TOP;
    }

    public SignLattice opposite(SignLattice signlattice){
        if (signlattice == SignLattice.NEG)
            return SignLattice.POS;
        else if (signlattice == SignLattice.POS)
            return SignLattice.NEG;
        else if (signlattice == SignLattice.ZERO)
            return SignLattice.ZERO;
        else if (signlattice == SignLattice.TOP)
            return SignLattice.TOP;
        else if (signlattice == SignLattice.BOTTOM)
            return SignLattice.BOTTOM;

        return SignLattice.TOP;
    }

    public SignLattice evalAdditionSign(SignLattice left, SignLattice right){
        if( (left == SignLattice.POS && right == SignLattice.NEG) || (left == SignLattice.NEG && right == SignLattice.POS) )
            return SignLattice.TOP;
        else if ( (left == SignLattice.POS && (right == SignLattice.POS || right == SignLattice.ZERO)) ||
                ( (left == SignLattice.POS || left == SignLattice.ZERO) && right == SignLattice.POS) )
            return SignLattice.POS;
        else if ( (left == SignLattice.NEG && (right == SignLattice.NEG || right == SignLattice.ZERO)) ||
    ...
    public SignLattice evalMultiplicationSign(SignLattice left, SignLattice right){
    ...
    public SignLattice evalSubtractionSign(SignLattice left, SignLattice right){
        if ( (left == SignLattice.POS && right == SignLattice.POS) || (left == SignLattice.NEG && right == SignLattice.NEG))
            return SignLattice.TOP;
    ...
    public SignLattice evalDivisionSign(SignLattice left, SignLattice right){
        if( (left == SignLattice.POS && right == SignLattice.POS) || (left == SignLattice.NEG && right == SignLattice.NEG))
            return SignLattice.POS;
        else if ( (left == SignLattice.POS && right == SignLattice.NEG) || (left == SignLattice.NEG && right == SignLattice.POS) )
            return SignLattice.NEG;
        else if ( left == SignLattice.BOTTOM || right == SignLattice.BOTTOM || right == SignLattice.ZERO )
            return SignLattice.BOTTOM;
        else if ( left == SignLattice.TOP || right == SignLattice.TOP)
            return SignLattice.TOP;
        else if ( left == SignLattice.ZERO)
    ...
    public SignLattice evalModuloSign(SignLattice left, SignLattice right){
        return right;
    }

    public SignLattice evalRemainderSign(SignLattice left, SignLattice right){
        return left;
    }
