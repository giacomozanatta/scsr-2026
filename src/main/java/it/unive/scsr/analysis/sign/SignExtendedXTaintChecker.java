...
public class SignExtendedXTaintChecker<H extends HeapValue<H>, T extends TypeValue<T>> implements
		SemanticCheck<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<LatticeProduct<ExtendedSignLattice, TaintThreeLevelsLattice>>, TypeEnvironment<T>>, SimpleAbstractDomain<HeapEnvironment<H>, ValueEnvironment<LatticeProduct<ExtendedSignLattice, TaintThreeLevelsLattice>>, TypeEnvironment<T>>> {

	@Override
	public boolean visit(
			SemanticTool<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<LatticeProduct<ExtendedSignLattice, TaintThreeLevelsLattice>>, TypeEnvironment<T>>, SimpleAbstractDomain<HeapEnvironment<H>, ValueEnvironment<LatticeProduct<ExtendedSignLattice, TaintThreeLevelsLattice>>, TypeEnvironment<T>>> tool,
	...
	private void process(
			...
			AnalyzedCFG<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<LatticeProduct<ExtendedSignLattice, TaintThreeLevelsLattice>>, TypeEnvironment<T>>> res) {
		if (descriptor.getAnnotations().contains(TaintThreeLevels.SINK_MATCHER)) { // check if the called function is annotated as sink
			for (int i = resolved.getCallType() == CallType.INSTANCE ? 1 : 0; i < uc.getParameters().length; i++) {
				...
				AnalysisState<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<LatticeProduct<ExtendedSignLattice, TaintThreeLevelsLattice>>, TypeEnvironment<T>>> postState = res
				...
				if (comExprIterator.hasNext()) {
					...
					try {
						...
						for (SymbolicExpression s : reachableIds) {
							...
							ValueEnvironment<LatticeProduct<ExtendedSignLattice, TaintThreeLevelsLattice>> valueState = postState.getExecutionState().valueState;
							SignExtendedXTaint signAnalysisValueDomain = (SignExtendedXTaint) tool.getAnalysis().domain.valueDomain;
