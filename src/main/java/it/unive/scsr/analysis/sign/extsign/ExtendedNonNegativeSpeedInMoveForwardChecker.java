...
/**
 * Semantic check : post-analysis pass that walks the CFG and raises warnings based on abstract values computed during the analysis.
 * Only looks at "moveForward" functions, and watches for setSpeed calls inside those.
 * Evaluates the abstract value of the first argument at that call site.
 * Issues a warning based on the abstract value.
 * @param <H>
 * @param <T>
 */
public class ExtendedNonNegativeSpeedInMoveForwardChecker<H extends HeapValue<H>, T extends TypeValue<T>> implements
	...
	@Override
	public boolean visit(SemanticTool<SimpleAbstractState<HeapEnvironment<H>,
									 	ValueEnvironment<ExtendedSignLattice>,
									 	TypeEnvironment<T>>,
									 	SimpleAbstractDomain<HeapEnvironment<H>,
											ValueEnvironment<ExtendedSignLattice>,
						...
						if (comExprIterator.hasNext()) {
							...
							try {
								...
								for (SymbolicExpression s : reachableIds) {
									...
									else if(abstractValue == ExtendedSignLattice.NEGZERO)
										tool.warnOn(c, "The speed is negative or zero within the function " + graph.getDescriptor().getName());
									else if(abstractValue == ExtendedSignLattice.TOP || abstractValue == ExtendedSignLattice.NOTZERO)
