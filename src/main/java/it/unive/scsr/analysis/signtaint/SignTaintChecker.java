...
public class SignTaintChecker<H extends HeapValue<H>, T extends TypeValue<T>> implements
	...
	private void process(
		...
		if (descriptor.getAnnotations().contains(SignTaint.SINK_MATCHER)) { // check if the called function is annotated as sink
			for (int i = resolved.getCallType() == CallType.INSTANCE ? 1 : 0; i < uc.getParameters().length; i++) {
				Expression par = uc.getParameters()[i];
				AnalysisState<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<SignTaintLattice>, TypeEnvironment<T>>> postState = res
				...
				if (comExprIterator.hasNext()) {
					...
					try {
						...
						for (SymbolicExpression s : reachableIds) {
							...
							SignTaint signAnalysisValueDomain = (SignTaint) tool.getAnalysis().domain.valueDomain;
