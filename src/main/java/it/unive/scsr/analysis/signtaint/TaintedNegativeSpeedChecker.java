 ...
 */
public class TaintedNegativeSpeedChecker<H extends HeapValue<H>, T extends TypeValue<T>> implements
        SemanticCheck<
        ...
        for (var res : tool.getResultOf(graph)) {
            ...
            try {
                ...
                for (SymbolicExpression s : reachableIds) {
                    ...
                    SignTaintLattice av       = evalProduct(tool, postState, s, c);
                    SignLattice signPart  = av.first;
                    ...
                    if (signPart == SignLattice.NEG)
                        tool.warnOn(c, "[SIGN] Speed is negative in " + fn);
                    else if (signPart == SignLattice.TOP)
                        tool.warnOn(c, "[SIGN] Speed may be negative in " + fn);

                    // Check 3: combined sign + taint warning
                    boolean possiblyNegative = signPart == SignLattice.NEG || signPart == SignLattice.TOP;
