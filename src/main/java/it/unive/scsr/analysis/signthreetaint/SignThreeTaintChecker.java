...
import it.unive.scsr.analysis.taint.*;
import it.unive.lisa.analysis.informationFlow.BaseTaint;
import it.unive.scsr.analysis.signthreetaint.SignThreeTaintLattice;
...
// It is the same as the TaintThreeLevelsChecker but it is adapted to the SignThreeTaint domain, so it checks if the abstract value of the parameter is possibly or always tainted, instead of checking if it is clean, tainted or unknown.
public class SignThreeTaintChecker<H extends HeapValue<H>, T extends TypeValue<T>> implements
	...
	private void process(
		...
		if (descriptor.getAnnotations().contains(SignThreeTaint.SINK_MATCHER)) { // check if the called function is annotated as sink
				...
				if (comExprIterator.hasNext()) {
					...
					try {
						...
						for (SymbolicExpression s : reachableIds) {
							...
							SignThreeTaint signAnalysisValueDomain = (SignThreeTaint) tool.getAnalysis().domain.valueDomain;
							SemanticOracle oracle = tool.getAnalysis().domain.makeOracle(postState.getExecutionState());
							SignThreeTaintLattice abstractValue = signAnalysisValueDomain.eval(valueState, (ValueExpression) s,
							...
							if (abstractValue.isAlwaysTainted())
								...
								tool.warnOn(uc, "There is a possible taint value in a sink: " + par.getLocation());
						}
					} catch (SemanticException e) {
						e.printStackTrace();
					}
