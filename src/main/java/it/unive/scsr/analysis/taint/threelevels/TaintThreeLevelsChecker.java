				...
				if (comExprIterator.hasNext()) {
					...
					try {
						...
						for (SymbolicExpression s : reachableIds) {
							...
							TaintThreeLevels taintAnalysisValueDomain = (TaintThreeLevels) tool.getAnalysis().domain.valueDomain;
							SemanticOracle oracle = tool.getAnalysis().domain.makeOracle(postState.getExecutionState());
							TaintThreeLevelsLattice abstractValue = taintAnalysisValueDomain.eval(valueState, (ValueExpression) s,
									(ProgramPoint) uc, oracle);

							if (abstractValue.isAlwaysTainted()) {
								tool.warnOn(uc, "Definite warning: parameter marked as Taint");
							} else if (abstractValue.isPossiblyTainted()) {
								tool.warnOn(uc, "Possible warning: parameter marked as Top");
							}
						}
					} catch (SemanticException e) {
						e.printStackTrace();
