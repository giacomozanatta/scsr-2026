...
public class TaintThreeLevelsCheckerSolution<H extends HeapValue<H>, T extends TypeValue<T>> implements
        SemanticCheck<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<TaintThreeLevelsLatticeSolution>, TypeEnvironment<T>>, SimpleAbstractDomain<HeapEnvironment<H>, ValueEnvironment<TaintThreeLevelsLatticeSolution>, TypeEnvironment<T>>> {

    @Override
    public boolean visit(
            SemanticTool<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<TaintThreeLevelsLatticeSolution>, TypeEnvironment<T>>, SimpleAbstractDomain<HeapEnvironment<H>, ValueEnvironment<TaintThreeLevelsLatticeSolution>, TypeEnvironment<T>>> tool,
    ...
    private void process(
            ...
            AnalyzedCFG<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<TaintThreeLevelsLatticeSolution>, TypeEnvironment<T>>> res) {
        if (descriptor.getAnnotations().contains(TaintThreeLevels.SINK_MATCHER)) { // check if the called function is annotated as sink
            for (int i = resolved.getCallType() == CallType.INSTANCE ? 1 : 0; i < uc.getParameters().length; i++) {
                ...
                AnalysisState<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<TaintThreeLevelsLatticeSolution>, TypeEnvironment<T>>> postState = res
                ...
                if (comExprIterator.hasNext()) {
                    ...
                    try {
                        ...
                        for (SymbolicExpression s : reachableIds) {
                            ...
                            ValueEnvironment<TaintThreeLevelsLatticeSolution> valueState = postState.getExecutionState().valueState;
                            TaintThreeLevelsSolution signAnalysisValueDomain = (TaintThreeLevelsSolution) tool.getAnalysis().domain.valueDomain;
                            SemanticOracle oracle = tool.getAnalysis().domain.makeOracle(postState.getExecutionState());
                            TaintThreeLevelsLatticeSolution abstractValue = signAnalysisValueDomain.eval(valueState, (ValueExpression) s,
                                    (ProgramPoint) uc, oracle);

                            //check the abstractValue of the parameter
                            if (abstractValue == TaintThreeLevelsLatticeSolution.TAINTED)
                                tool.warnOn(uc, "There is a definite tainted value in a sink: " + par.getLocation());
                            else if (abstractValue == TaintThreeLevelsLatticeSolution.TOP)
