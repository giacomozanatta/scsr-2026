...
/**
 * It checks if there are calls where their parameters contain string values with the Suffix "http://" or "https://"
 */
public class DotComStringChecker <H extends HeapValue<H>, T extends TypeValue<T>> implements
SemanticCheck<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<StrSuffix>, TypeEnvironment<T>>, SimpleAbstractDomain<HeapEnvironment<H>, ValueEnvironment<StrSuffix>, TypeEnvironment<T>>> {
	...
	private void process(
				...
				if (comExprIterator.hasNext()) {
					...
					try {
						...
						for (SymbolicExpression s : reachableIds) {
							...
							if (abstractValue.suffix.endsWith(".com"))
								paramsToWarn[i] = true;
						}
					} catch (SemanticException e) {
						e.printStackTrace();
			...
			if(requireWarning(paramsToWarn))
				tool.warnOn(uc, "The function uses a .com in the following parameters: " + prettyPrintParamsToWarn(paramsToWarn));
