...
import it.unive.scsr.analysis.taintedsign.TaintedSignLattice;
...
// passed to that call and emits a warning when its value is tainted.
public class TaintedSignChecker<H extends HeapValue<H>, T extends TypeValue<T>> implements
        SemanticCheck<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<TaintedSignLattice>, TypeEnvironment<T>>, SimpleAbstractDomain<HeapEnvironment<H>, ValueEnvironment<TaintedSignLattice>, TypeEnvironment<T>>> {

    @Override
    public boolean visit(
            SemanticTool<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<TaintedSignLattice>, TypeEnvironment<T>>, SimpleAbstractDomain<HeapEnvironment<H>, ValueEnvironment<TaintedSignLattice>, TypeEnvironment<T>>> tool,
    ...
    private void process(
            ...
            AnalyzedCFG<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<TaintedSignLattice>, TypeEnvironment<T>>> res) {
        System.out.println("annot: " + descriptor.getAnnotations());
        if (descriptor.getAnnotations().contains(TaintThreeLevels.SINK_MATCHER)) { // check if the called function is annotated as sink
            ...
            for (int i = resolved.getCallType() == Call.CallType.INSTANCE ? 1 : 0; i < uc.getParameters().length; i++) {
                ...
                AnalysisState<SimpleAbstractState<HeapEnvironment<H>, ValueEnvironment<TaintedSignLattice>, TypeEnvironment<T>>> postState = res
                ...
                if (comExprIterator.hasNext()) {
                    ...
                    try {
                        ...
                        for (SymbolicExpression s : reachableIds) {
                            ...
                            ValueEnvironment<TaintedSignLattice> valueState = postState.getExecutionState().valueState;
                            TaintedSign taintedSignAnalysisValueDomain = (TaintedSign) tool.getAnalysis().domain.valueDomain;
                            SemanticOracle oracle = tool.getAnalysis().domain.makeOracle(postState.getExecutionState());
                            TaintedSignLattice abstractValue = taintedSignAnalysisValueDomain.eval(valueState, (ValueExpression) s,
                                    (ProgramPoint) uc, oracle);

                            System.out.println("out: " + abstractValue);
                            //check the abstractValue of the parameter
                            if (abstractValue.second == TaintThreeLevelsLattice.TAINT)
                                tool.warnOn(uc, "There is a taint value in a sink: " + par.getLocation());
                            if (abstractValue.second == TaintThreeLevelsLattice.TOP)
