...
import it.unive.scsr.analysis.sign_x_threetaint.ExtendedSignXTaintThreeLevelsChecker;
import it.unive.scsr.analysis.sign_x_threetaint.ExtendedSignXThreeTaint;
...
public class ExtendedSignXThreeTaintAnalysisTest {
    ...
    @Test
    public void testSignXTaintAnalysis() throws ParsingException, AnalysisException {
        ...
        conf.workdir = "outputs/extendedsign_x_taintthreelevels";

        // we specify the visual format of the analysis results
        //conf.outputs.add(new HtmlInputs(true));
        conf.outputs.add(new HtmlResults<>(true));
        // we specify the analysis that we want to execute
        conf.analysis = simpleDomain(new PointBasedHeap(), new ExtendedSignXThreeTaint(), defaultTypeDomain());
        ...
        for(CFG cfg : program.getAllCFGs()) {
        	...
        	if(isSource(name))
        		...
        		cfg.getDescriptor().addAnnotation(ExtendedSignXThreeTaint.SINK_ANNOTATION);
        }
    
        // added checker to the analysis
        conf.semanticChecks.add(new ExtendedSignXTaintThreeLevelsChecker<>());
