...
import it.unive.scsr.analysis.Extended_Cartesian_Product.ExtendedSign_X_ThreeTaint;
...
public class ExtendedSign_X_ThreeTaintTest {
    ...
    public void testTaintAnalysis() throws ParsingException, AnalysisException {
        ...
        Program program = IMPFrontend.processFile("inputs/extendedSign_X_ThreeTaint.imp");

        // we build a new configuration for the analysis
        LiSAConfiguration conf = new DefaultConfiguration();

        // we specify where we want files to be generated
        conf.workdir = "outputs/extendedSign_X_ThreeTaint";

        // we specify the visual format of the analysis results
        //conf.outputs.add(new HtmlInputs(true));
        conf.outputs.add(new HtmlResults<>(true));
        // we specify the analysis that we want to execute
        conf.analysis = simpleDomain(defaultHeapDomain(), new ExtendedSign_X_ThreeTaint(), defaultTypeDomain());
