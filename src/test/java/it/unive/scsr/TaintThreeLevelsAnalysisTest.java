...
public class TaintThreeLevelsAnalysisTest {
	...
	private final String[] namePossibleSource = {"possibleSource", "mayTint"};
    ...
    public void testTaintAnalysis() throws ParsingException, AnalysisException {
        ...
        Program program = IMPFrontend.processFile("inputs/taintTL.imp");

        // we build a new configuration for the analysis
        LiSAConfiguration conf = new DefaultConfiguration();

        // we specify where we want files to be generated
        conf.workdir = "outputs/taintTL";
	...
	private boolean isPossibleSource(String name) {
		for(String sink : namePossibleSource)
			if(sink.equals(name))
				return true;
		return false;
	}
